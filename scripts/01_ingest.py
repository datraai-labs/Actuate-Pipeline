...
def run(session_path: Path) -> dict:
    ...
    if raw_imu_json.exists():
        ...
        if isinstance(imu_data, list) and len(imu_data) > 0 and isinstance(imu_data[0], dict):
            ...
            imu_df = pd.DataFrame(rows, columns=expected_cols)
        else:
    ...
    imu_df = imu_df[expected_cols].astype(np.float64).dropna()
    ...
    if video_meta.get("creation_time"):
        try:
            ...
            recording_start_epoch_ms = dt.timestamp() * 1000.0
            start_source = "video_creation_time"
        except (ValueError, TypeError):
            pass

    if recording_start_epoch_ms is None:
        recording_start_epoch_ms = float(imu_raw[0, 0])
        start_source = "imu_first_timestamp"

    print(f"[{STEP}] Recording start: {recording_start_epoch_ms:.0f}ms (source: {start_source})")

    # ─── Step 5: Write session_meta.json ──────────────────────
    duration_seconds = float(pts[-1]) if len(pts) > 0 else 0.0
