...
def test_mismatched_frames_partial_coverage_and_unmatched_stereo_fail(tmp_path):
    ...
    input_facts["timing"]["streams"][0].update(coverage_rows=2, outside_imu_coverage_rows=1)
    output = tmp_path / "qc.json"
    ...
    assert checks["camera_imu_coverage"] == {
        "check_id": "camera_imu_coverage",
        "result": "fail",
        "evidence": {
            "matched_camera_rows": 6,
            "rows_within_imu_coverage": 5,
