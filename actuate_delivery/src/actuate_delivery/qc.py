...
def supplier_issues(internal: dict) -> tuple[list[str], list[str]]:
    ...
    unresolved = [
        name for name, result in checks.items() if result not in ("pass", "not_applicable")
    ]
    blocked = [
        name
...
def controlled_limitations(internal: dict) -> list[str]:
    ...
    if any(check.startswith("camera_imu_coverage") for check in material):
        streams = [
            stream for stream in facts["timing"]["streams"] if stream["outside_imu_coverage_rows"]
        ]
        ...
        result.append(
            {
                ...
                "start_time_s": None if first["mp4_pts_ns"] is None else first["mp4_pts_ns"] / 1e9,
                "end_time_s": None if last["mp4_pts_ns"] is None else last["mp4_pts_ns"] / 1e9,
            }
        )
    return result
        ...
        facts.append(
            {
                ...
                "missing_sof_rows": sum(
                    row["mapping_status"] == "missing_sof" for row in stream_rows
                ),
                "video_only_rows": sum(
                    row["vts_match_status"] == "video_only" for row in stream_rows
                ),
...
def _supplier_qc(internal: dict, decision: dict) -> dict:
    ...
    supplier = {
        ...
        "integrity": {
            "raw_file_count": facts["source"]["file_count"],
        ...
        supplier["stereo"] = {
            ...
            "unmatched_ranges": {
                stream["camera_stream_id"]: next(
                    item
                    for item in facts["timing"]["streams"]
...
def build_qc(facts: dict, output: Path) -> QcArtifact:
    ...
    add(
        "capture_structure",
        "pass" if grouping == "complete" else "fail",
    ...
    add(
        "imu_decode",
        "pass"
        if imu["status"] == "decoded"
        else ("fail" if imu["status"] == "failed" else "unknown"),
        imu,
    )
    for stream in facts["streams"]:
        stream_id = stream["camera_stream_id"]
        ...
        add(
            f"vts_decode:{stream_id}",
            "pass"
            if vts["status"] == "decoded"
            else ("fail" if vts["status"] == "failed" else "unknown"),
            vts,
        )
        add(
            f"video_decode:{stream_id}",
            "pass"
            if video["status"] == "verified"
            else ("fail" if video["status"] == "failed" else "unknown"),
            video,
        )
        comparable = vts["status"] == "decoded" and video["status"] == "verified"
        counts = {
            "vts_frame_count": vts.get("frame_count"),
            "video_frame_count": video.get("frame_count"),
        }
        add(
            f"video_vts_frame_count:{stream_id}",
            "pass"
            if comparable and counts["vts_frame_count"] == counts["video_frame_count"]
            else "fail"
            if comparable
            else "unknown",
            counts,
    ...
    add(
        ...
        else ("fail" if timing["status"] == "failed" else "unknown"),
        timing,
    )
    ready = timing["status"] == "ready"
    expected_rows = sum(
        max(stream["vts"].get("frame_count") or 0, stream["video"].get("frame_count") or 0)
        for stream in facts["streams"]
    )
    add(
        "timing_row_accounting",
        "pass"
        if ready and timing["row_count"] == expected_rows
        else "fail"
        if ready
        else "unknown",
    ...
    add(
        "camera_imu_coverage",
        "pass"
        if ready and matched and coverage == matched
        else "fail"
        if ready and matched
        else "unknown",
        {
            "matched_camera_rows": matched,
            "rows_within_imu_coverage": coverage,
            "rows_outside_imu_coverage": None
            if matched is None or coverage is None
    ...
    for stream in timing.get("streams", ()):
        ...
        add(
            f"camera_imu_coverage:{stream['camera_stream_id']}",
            "pass"
            if stream_matched and stream_coverage == stream_matched
            else "fail"
            if stream_matched
            else "unknown",
            stream,
    ...
    add(
        "stereo_sequence_pairing",
        stereo_result,
        {
            "stereo_pair_count": timing.get("stereo_pair_count"),
    ...
    add(
        "telemetry",
        "not_applicable"
        if telemetry["status"] == "absent"
        else ("pass" if telemetry["status"] == "decoded" else "fail"),
        telemetry,
    )
    counts = {
    ...
    document = {
        "schema_version": "actuate_delivery.qc_internal.v2",
        "capture_id": facts["capture_id"],
        "facts": facts,
        "checks": checks,
        "summary": counts,
    }
    ...
    try:
        ...
        staging.unlink(missing_ok=True)
    return QcArtifact(
        output_hash, counts["pass"], counts["fail"], counts["unknown"], counts["not_applicable"]
    )
