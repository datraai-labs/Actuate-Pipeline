...
def _calibration(calibration, episode_ids):
    if calibration is None:
        return None
    required = {"schema_version", "calibration_id", "rig_id", "applies_to_episode_ids",
                "applied_by_pipeline", "cameras", "transforms", "imu"}
...
def project_supplier(
    ...
    if any(len(value) != 14 or not value.startswith("episode_") or not value[8:].isdigit()
           or value == "episode_000000" for value in episode_ids):
    ...
    for episode in sorted(episodes, key=lambda item: item["episode_id"]):
        ...
        for index, member in enumerate(visualizations, 1):
            name = "capture_preview.mp4" if len(visualizations) == 1 else f"capture_preview_{index}.mp4"
            previews.append({
                "path": f"previews/{name}", "byte_count": member["size_bytes"],
...
def build_delivery(run_dir: Path, projection: Path, output: Path) -> DeliveryArtifact:
    ...
    try:
        ...
        calibration_files = list((projection / "calibration").glob("*.json")) \
            if (projection / "calibration").is_dir() else []
        ...
        for row in rows:
            ...
            projected_files.update((f"episodes/{episode_id}/meta.json",
                                    f"episodes/{episode_id}/derived/qc.json"))
        actual_projection = {
            ...
            with sqlite3.connect(run_dir / "run.sqlite") as database:
                ...
                for row in rows:
                    ...
                    for preview_index, member in enumerate(visualizations, 1):
                        name = ("capture_preview.mp4" if len(visualizations) == 1
                                else f"capture_preview_{preview_index}.mp4")
                        preview = {"path": f"previews/{name}", "byte_count": member["size_bytes"],
                                   "sha256": member["source_sha256"]}
                        expected_previews.append(preview)
                        target = target_episode / preview["path"]
                        _materialize(run_dir / "cache/blobs" / preview["sha256"], target,
                                     preview["sha256"], preview["byte_count"])
